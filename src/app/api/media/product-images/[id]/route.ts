...
export async function GET(
  ...
  if (!record) {
    return NextResponse.json({ error: 'Product gallery image not found' }, { status: 404 });
  }
  ...
  const fallback = await serveFromPath(record.image_url, {
  ...
  return NextResponse.json({ error: 'Product gallery asset missing' }, { status: 404 });
}
