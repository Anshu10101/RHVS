...
export async function GET(
  ...
  const rows = await executeQuery(
    `SELECT image_blob, image_mime, image_size, image_original_name, image_path, is_active
     FROM hero_images
     WHERE id = ?
     LIMIT 1`,
    [id]
  ...
  if (!record || record.is_active === false || record.is_active === 0) {
  ...
  return NextResponse.json({ error: 'Hero image asset missing' }, { status: 404 });
}
...
async function serveFromPath(
  ...
  // Fallback for legacy filesystem paths
  ...
  // External URLs or other paths – issue redirect
