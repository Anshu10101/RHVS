...
interface NewsEditorProfileRow {
...
export async function GET(
  ...
  const rows = await executeQuery(
    `SELECT profile_photo_blob, profile_photo_path
     ...
     LIMIT 1`,
    [id]
  ) as NewsEditorProfileRow[];

  const record = rows?.[0];

  if (!record) {
    return NextResponse.json({ error: 'News editor profile photo not found' }, { status: 404 });
  }
  ...
  if (hasBlob && record.profile_photo_blob) {
    const headers: Record<string, string> = {
      ...
      'Content-Disposition': `inline; filename="news-editor-${id}-profile.jpg"`,
  ...
  return NextResponse.json({ error: 'News editor profile asset missing' }, { status: 404 });
}
